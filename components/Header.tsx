...
const Header: React.FC<HeaderProps> = ({ onSearchChange, onListClick, viewMode, onToggleView }) => {
  return (
    <header className="fixed top-0 left-0 right-0 z-40 glass border-b border-slate-200">
      <div className="container mx-auto px-4 h-20 flex items-center justify-between gap-4">
        {/* Logo */}
        <div className="flex items-center gap-2 cursor-pointer" onClick={() => onToggleView('marketplace')}>
        ...
        {viewMode === 'marketplace' ? (
          ...
          <div className="flex-1 max-w-xl text-center">
            <span className="text-sm font-bold text-indigo-600 bg-indigo-50 px-4 py-1.5 rounded-full uppercase tracking-widest">
              Admin Control Panel
            </span>
          </div>
        )}
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          <button 
            onClick={() => onToggleView(viewMode === 'marketplace' ? 'admin' : 'marketplace')}
            className={`px-4 py-2.5 rounded-xl font-semibold transition-all border ${
              viewMode === 'admin' 
                ? 'bg-slate-800 text-white border-slate-800' 
                : 'bg-white text-slate-700 border-slate-200 hover:bg-slate-50'
            }`}
          >
            <i className={`fa-solid ${viewMode === 'admin' ? 'fa-shop' : 'fa-gauge-high'} mr-2`}></i>
            <span className="hidden sm:inline">{viewMode === 'admin' ? 'Marketplace' : 'Admin'}</span>
          </button>
